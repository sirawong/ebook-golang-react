...
func (b userRepositoryDB) UploadImage(image io.Reader) (*Image, error) {
	ctx := context.Background()
	resp, err := b.cld.Upload.Upload(ctx, image, uploader.UploadParams{Folder: "user", UseFilename: true, UniqueFilename: true})
	if err != nil {
		return nil, err
	}
...
func (b userRepositoryDB) DeleteImage(image Image) error {
	ctx := context.Background()
