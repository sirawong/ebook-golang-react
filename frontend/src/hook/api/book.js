  ...
  return new Promise((resolve, reject) => {
    axios
      ...
      .then(async (res) => {
        if (image.raw[0]) {
          await axios
            .post(`localhost:8001/admin/${res.data.id}/image`, formData, {
              headers: {
...
export async function deleteBook(bookid) {
  ...
  return new Promise((resolve, reject) => {
    axios
      .delete(`localhost:8001/admin/${bookid}`, { withCredentials: true })
      .then(async () => {
        resolve();
      })
      .catch((error) => {
...
export async function uploadBookImage(file, bookid) {
  ...
  return new Promise((resolve, reject) => {
    axios
      .post(`localhost:8001/admin/${bookid}/image`, formData, {
        headers: {
          'Content-Type': 'multipart/form-data',
        },
        withCredentials: true,
